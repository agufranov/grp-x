(() => {
  ...
  function drawPlot(svg,raw,{robust=false,color='#1e6c5b'}={}){
    ...
    markup+=`<path d="${path(data)}" clip-path="url(#${clipId})" fill="none" stroke="${color}" stroke-width="1.3" vector-effect="non-scaling-stroke"/>`;
  ...
  function clampCepstrumOffset(scale,value){const edge=1-1/scale;return Math.max(Math.min(0,edge),Math.min(Math.max(0,edge),value));}
  function setCepstrumView(svg,state,{scale=state.scale,offset=state.offset}={}){if(!Number.isFinite(scale)||!Number.isFinite(offset))return;scale=Math.max(CEPSTRUM_SCALE_MIN,Math.min(CEPSTRUM_SCALE_MAX,scale));offset=clampCepstrumOffset(scale,offset);if(scale===state.scale&&offset===state.offset)return;state.scale=scale;state.offset=offset;updateCepstrumScaleHeader(svg,state);scheduleCepstrumScaleRedraw(svg,state);}
  function resetCepstrumView(svg,state){setCepstrumView(svg,state,{scale:CEPSTRUM_SCALE_DEFAULT,offset:0});}
  function cepstrumPointerFraction(svg,event){const matrix=svg.getScreenCTM();if(!matrix)return NaN;const point=svg.createSVGPoint();point.x=event.clientX;point.y=event.clientY;const x=point.matrixTransform(matrix.inverse()).x,width=svg.viewBox.baseVal.width;return(x-CEPSTRUM_PLOT_LEFT)/(width-CEPSTRUM_PLOT_LEFT-CEPSTRUM_PLOT_RIGHT);}
