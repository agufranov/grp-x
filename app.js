(() => {
  ...
  const CEPSTRUM_SCALE_MIN=1,CEPSTRUM_SCALE_MAX=32,CEPSTRUM_SCALE_DOUBLE_DISTANCE=160;
  ...
  function drawMiniWell(ball){
    const deepest=Math.max(...M.points.map(p=>p.depth),M.nkt),maxDepth=Math.ceil(deepest/500)*500,w=Math.max(240,E.miniWell.clientWidth||E.wellHead.clientWidth),h=WELL_GEOMETRY.miniHeight,y=WELL_GEOMETRY.miniAxis,L=62,R=w-20,x=d=>L+d/maxDepth*(R-L),nktX=x(M.nkt),ballX=ball?x(ball.depth):R+1,shift=y-WELL_GEOMETRY.axis,sy=value=>value+shift;
    let markup='<defs><linearGradient id="miniPipe" x1="0" x2="0" y1="0" y2="1"><stop stop-color="#dce8e3"/><stop offset=".5" stop-color="#51665e"/><stop offset="1" stop-color="#94a69f"/></linearGradient><pattern id="miniHatch" width="8" height="8" patternUnits="userSpaceOnUse" patternTransform="rotate(45)"><line y2="8" stroke="#fff" stroke-opacity=".08" stroke-width="3"/></pattern></defs>';
    markup+=`<rect x="${L}" y="${sy(136)}" width="${R-L}" height="28" rx="14" fill="url(#miniPipe)"/><rect x="${ballX}" y="${sy(118)}" width="${Math.max(0,R-ballX)}" height="64" fill="#10191d" opacity=".68"/><rect x="${ballX}" y="${sy(118)}" width="${Math.max(0,R-ballX)}" height="64" fill="url(#miniHatch)"/><line x1="${L}" y1="${y}" x2="${nktX}" y2="${y}" stroke="#688df2" stroke-width="7"/><line x1="${nktX}" y1="${sy(WELL_GEOMETRY.tickStart)}" x2="${nktX}" y2="${h}" stroke="#86a3f5" stroke-width="4"/><circle cx="${nktX}" cy="${y}" r="7" fill="#b9c9f7"/>`;
    ...
    E.miniWell.setAttribute('viewBox',`0 0 ${w} ${h}`);E.miniWell.innerHTML=markup;
  }
  ...
  function setCepstrumScale(svg,state,value){let scale=Math.max(CEPSTRUM_SCALE_MIN,Math.min(CEPSTRUM_SCALE_MAX,value));if(scale<1.005)scale=CEPSTRUM_SCALE_MIN;if(scale===state.scale)return;state.scale=scale;updateCepstrumScaleHeader(svg,state);scheduleCepstrumScaleRedraw(svg,state);}
  function bindCepstrumScale(svg){const state={scale:CEPSTRUM_SCALE_MIN,raw:[],options:null,drag:null,frame:0};cepstrumScaleStates.set(svg,state);updateCepstrumScaleHeader(svg,state);svg.addEventListener('pointerdown',event=>{if(event.pointerType==='touch'||event.button!==0)return;event.preventDefault();state.drag={pointerId:event.pointerId,startX:event.clientX,startScale:state.scale};svg.setPointerCapture(event.pointerId);svg.classList.add('is-scaling');document.documentElement.classList.add('is-scaling-cepstrum');});svg.addEventListener('pointermove',event=>{if(!state.drag||state.drag.pointerId!==event.pointerId)return;const distance=event.clientX-state.drag.startX;setCepstrumScale(svg,state,state.drag.startScale*2**(distance/CEPSTRUM_SCALE_DOUBLE_DISTANCE));});const finish=event=>{if(!state.drag||state.drag.pointerId!==event.pointerId)return;state.drag=null;svg.classList.remove('is-scaling');document.documentElement.classList.remove('is-scaling-cepstrum');if(svg.hasPointerCapture(event.pointerId))svg.releasePointerCapture(event.pointerId);};svg.addEventListener('pointerup',finish);svg.addEventListener('pointercancel',finish);svg.addEventListener('lostpointercapture',finish);}
  ...
  document.addEventListener('click',event=>{const reset=event.target.closest('.chart-scale-reset');if(reset){const svg=reset.closest('.cepstrum-card').querySelector('.cepstrum-scale-target'),state=cepstrumScaleStates.get(svg);setCepstrumScale(svg,state,CEPSTRUM_SCALE_MIN);return;}const head=event.target.closest('.chart-head');if(!head||event.target.closest('.chart-scale'))return;const card=head.closest('.cepstrum-card'),collapsed=card.classList.toggle('collapsed');head.querySelector('.chart-toggle').setAttribute('aria-expanded',String(!collapsed));if(!collapsed&&M)requestAnimationFrame(()=>drawWell());});
