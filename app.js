(() => {
  ...
  function drawMiniWell(ball){const deepest=Math.max(...M.points.map(p=>p.depth),M.nkt),maxDepth=Math.ceil(deepest/500)*500,w=Math.max(240,E.miniWell.clientWidth||E.wellHead.clientWidth-82),h=36,y=18,L=0,R=w,x=d=>L+d/maxDepth*(R-L),nktX=x(M.nkt),ballX=ball?x(ball.depth):R+1;let markup=`<rect x="${L}" y="14" width="${R-L}" height="8" rx="4" fill="#82908b"/><rect x="${ballX}" y="9" width="${Math.max(0,R-ballX)}" height="18" rx="2" fill="#121d21" opacity=".78"/><line x1="${L}" y1="${y}" x2="${nktX}" y2="${y}" stroke="#7597ef" stroke-width="3"/><line x1="${nktX}" y1="10" x2="${nktX}" y2="26" stroke="#9fb5f1" stroke-width="2"/>`;for(const p of M.points){const px=x(p.depth),muted=ball&&p.depth>ball.depth,color=muted?'#66736e':'#c5e77b';markup+=`<line x1="${px}" y1="8" x2="${px}" y2="28" stroke="${color}" stroke-width="2"/><circle cx="${px}" cy="${y}" r="3.5" fill="${color}" stroke="#29343a" stroke-width="1.5"/>`;}if(ball)markup+=`<circle cx="${ballX}" cy="${y}" r="7" fill="#ef8058" stroke="#ffd0bf" stroke-width="2"/>`;E.miniWell.setAttribute('viewBox',`0 0 ${w} ${h}`);E.miniWell.innerHTML=markup;}
