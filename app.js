(() => {
  ...
  const CEPSTRUM_SCALE_DEFAULT=1,CEPSTRUM_SCALE_MIN=.1,CEPSTRUM_SCALE_MAX=32,CEPSTRUM_SCALE_DOUBLE_DISTANCE=160;
  ...
  function setCepstrumScale(svg,state,value){let scale=Math.max(CEPSTRUM_SCALE_MIN,Math.min(CEPSTRUM_SCALE_MAX,value));if(Math.abs(scale-CEPSTRUM_SCALE_DEFAULT)<.005)scale=CEPSTRUM_SCALE_DEFAULT;if(scale===state.scale)return;state.scale=scale;updateCepstrumScaleHeader(svg,state);scheduleCepstrumScaleRedraw(svg,state);}
  function positionScaleTooltip(event){const x=Math.min(event.clientX+12,window.innerWidth-120),y=Math.min(event.clientY+14,window.innerHeight-28);E.scaleTooltip.style.transform=`translate3d(${Math.max(4,x)}px,${Math.max(4,y)}px,0)`;}
