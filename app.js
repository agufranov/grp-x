(() => {
  ...
  const INTERPOLATION=globalThis.GrpInterpolation,INTERPOLATION_DEFAULT=INTERPOLATION.DEFAULT_METHOD,INTERPOLATION_LABELS={pchip:'монотонный сплайн',linear:'линейная интерполяция'},SIM_PROCESSING=globalThis.GrpSimulationProcessing;
  ...
  let guideFrame=0,guideX=NaN,guideVisible=false;
  ...
  const fmtTime = (n,span=0) => `${Number(n).toLocaleString('ru-RU',{maximumFractionDigits:Math.abs(span)<1?3:2})} с`;
  ...
  function selectedSimulationPort(){const port=num(E.simPort.value);return isNum(port)?port:null;}

  function drawWellDiagram(ball){
    const deepest=Math.max(...M.points.map(p=>p.depth),M.nkt),maxDepth=Math.ceil(deepest/500)*500,{w,L,R,x,start,end}=wellHorizontalGeometry(E.svg.clientWidth,320,maxDepth),h=WELL_GEOMETRY.height,y=WELL_GEOMETRY.axis,nx=x(M.nkt),bx=ball?x(ball.depth):end+1,activePort=selectedSimulationPort(),activePoint=M.points.find(point=>point.port===activePort);
    ...
    M.points.forEach((p,i)=>{const px=x(p.depth),active=p.port===activePort,muted=ball&&p.depth>ball.depth,lower=i%2!==0,ly=lower?86:53,lend=lower?111:76,col=active?'#e1f79d':muted?'#68756f':'#c5e77b',tc=active?'#fffbd8':muted?'#7d8984':'#edf3ee';s+=`<line x1="${px}" y1="${WELL_GEOMETRY.tickStart}" x2="${px}" y2="${WELL_GEOMETRY.bottom}" stroke="${col}" stroke-width="${active?5:3}"/><circle cx="${px}" cy="${y}" r="8" fill="${col}" stroke="#29343a" stroke-width="3"/><line x1="${px}" y1="124" x2="${px}" y2="${lend}" stroke="#fff" stroke-opacity="${active ? .32 : .14}"/><text x="${px}" y="${ly}" text-anchor="middle" fill="${tc}" font-size="13" font-weight="${active?900:700}">№ ${p.port}</text><text x="${px}" y="${ly+15}" text-anchor="middle" fill="${active?'#dce9a6':'#91a29c'}" font-size="10">${fmt(p.depth)} м</text>`;});
    if(ball)s+=`<circle cx="${bx}" cy="${y}" r="16" fill="#ef8058" stroke="#ffd0bf" stroke-width="3"/><circle cx="${bx-4}" cy="${y-4}" r="4" fill="#ffc2aa"/>`;
    if(activePoint){const activeX=x(activePoint.depth),atBall=ball&&activePoint.port===ball.port;s+=`<circle class="active-simulation-port" data-port="${activePoint.port}" cx="${activeX}" cy="${y}" r="${atBall?21:13}" fill="none" stroke="#f5f7a8" stroke-width="3"/>`;}
    E.svg.setAttribute('viewBox',`0 ${WELL_GEOMETRY.top} ${w} ${h}`);E.svg.style.minWidth='0';E.svg.innerHTML=s+'</g>';
  }

  function drawMiniWell(ball){
    const deepest=Math.max(...M.points.map(p=>p.depth),M.nkt),maxDepth=Math.ceil(deepest/500)*500,{w,L,R,x,start,end}=wellHorizontalGeometry(E.miniWell.clientWidth||E.wellHead.clientWidth,240,maxDepth),h=WELL_GEOMETRY.miniHeight,y=WELL_GEOMETRY.miniAxis,nktX=x(M.nkt),ballX=ball?x(ball.depth):end+1,shift=y-WELL_GEOMETRY.axis,sy=value=>value+shift,activePort=selectedSimulationPort(),activePoint=M.points.find(point=>point.port===activePort);
    ...
    for(const p of M.points){const px=x(p.depth),active=p.port===activePort,muted=ball&&p.depth>ball.depth,color=active?'#e1f79d':muted?'#68756f':'#c5e77b';markup+=`<line x1="${px}" y1="${sy(WELL_GEOMETRY.tickStart)}" x2="${px}" y2="${h}" stroke="${color}" stroke-width="${active?5:3}"/><circle cx="${px}" cy="${y}" r="8" fill="${color}" stroke="#29343a" stroke-width="3"/>`;}
    ...
    if(activePoint){const activeX=x(activePoint.depth),atBall=ball&&activePoint.port===ball.port;markup+=`<circle class="active-simulation-port" data-port="${activePoint.port}" cx="${activeX}" cy="${y}" r="${atBall?21:13}" fill="none" stroke="#f5f7a8" stroke-width="3"/>`;}
  ...
  function tukeyWindow(values,alpha=.12,edges='both'){
    const result=Float64Array.from(values),edge=alpha*(values.length-1)/2;if(!(edge>0))return result;
    for(let i=0;i<values.length;i++){const distance=edges==='right'?values.length-1-i:edges==='left'?i:Math.min(i,values.length-1-i),weight=distance>=edge?1:.5*(1-Math.cos(Math.PI*distance/edge));result[i]*=weight;}
    return result;
  }
  function cepstrumFromValues(values,step,options={}){
    ...
    const magnitudes=new Float64Array(n);for(let i=0;i<n;i++)magnitudes[i]=Math.hypot(re[i],im[i]);
    re.set(SIM_PROCESSING.stabilizedLogSpectrum(magnitudes,options));im.fill(0);fft(re,im,true);
    const result=[];for(let i=0;i<Math.floor(n/2);i++)result.push({x:i*step,y:re[i]});return result;
  }
  function processSignal(source,{start=-Infinity,end=Infinity,low=0,high=Infinity,interpolation=INTERPOLATION_DEFAULT,windowEdges='both'}={}){
    ...
    const windowed=tukeyWindow(filtered,.12,windowEdges),processed=Array.from(windowed,(value,i)=>[grid.xmin+i*grid.step,value]);
    return{grid,processed,windowed};
  }
  function analyzeSignal(source,settings={},cepstrumOptions={}){
    const processed=processSignal(source,settings);return processed?{grid:processed.grid,processed:processed.processed,cepstrum:cepstrumFromValues(processed.windowed,processed.grid.step,cepstrumOptions)}:null;
  }
  function setGraphState(svg,empty,isEmpty,message){empty.textContent=message;empty.classList.toggle('hidden',!isEmpty);empty.parentElement.classList.toggle('empty-chart',isEmpty);const wellScale=empty.parentElement.querySelector('.chart-well-scale');if(wellScale)wellScale.disabled=isEmpty;if(isEmpty){svg.innerHTML='';const state=cepstrumScaleStates.get(svg);if(state){state.raw=[];state.options=null;state.view=null;}}}
  function projectWellDepth(svg,w,L,R,depth){
    if(!M)return NaN;const deepest=Math.max(M.nkt,...M.points.map(point=>point.depth)),maxDepth=Math.ceil(deepest/500)*500,wellGeometry=wellHorizontalGeometry(E.svg.clientWidth,320,maxDepth),wellRect=E.svg.getBoundingClientRect(),plotRect=svg.getBoundingClientRect(),screenProjection=wellRect.width>0&&plotRect.width>0;
    if(!screenProjection)return L+(depth/maxDepth-wellViewState.offset)*wellViewState.scale*(R-L);const screenX=wellRect.left+wellGeometry.x(depth)/wellGeometry.w*wellRect.width;return(screenX-plotRect.left)*w/plotRect.width;
  }
  function wellObjectMarkerMarkup(svg,w,L,R,T,B,clipId,xmin,xmax){
    if(!M)return'';const test=M.tests[+E.select.value||0],ball=M.points.find(point=>point.port===test.ballPort),activePort=selectedSimulationPort(),project=depth=>projectWellDepth(svg,w,L,R,depth);
    let markup=`<g class="well-object-markers" clip-path="url(#${clipId})" pointer-events="none">`,nktX=project(M.nkt),nktTime=xmin+(nktX-L)/(R-L)*(xmax-xmin),nktTimeText=fmtTime(nktTime,xmax-xmin);
    markup+=`<g class="well-object-marker" data-kind="nkt" data-depth="${M.nkt}" data-time="${nktTime}"><title>Конец НКТ · ${fmt(M.nkt)} м · ${nktTimeText}</title><line x1="${nktX}" y1="${T}" x2="${nktX}" y2="${B}" stroke="#688df2" stroke-opacity=".48" stroke-dasharray="3 3"/><text x="${nktX}" y="${T+9}" text-anchor="middle" fill="#526fc4" stroke="#fffdfa" stroke-width="2" paint-order="stroke" font-size="7" font-weight="800">НКТ</text></g>`;
    M.points.forEach((point,index)=>{const px=project(point.depth),active=point.port===activePort,muted=ball&&point.depth>ball.depth,color=active?'#648d20':muted?'#8a928e':'#76983f',opacity=active ? .76 : muted ? .2 : .36,labelY=T+(index%2?18:9);markup+=`<g class="well-object-marker" data-kind="perforation" data-port="${point.port}" data-depth="${point.depth}" data-active="${active}"><title>Перфорация № ${point.port} · ${fmt(point.depth)} м${active?' · активная в симуляции':''}</title><line x1="${px}" y1="${T}" x2="${px}" y2="${B}" stroke="${color}" stroke-opacity="${opacity}" stroke-width="${active?2:1}" stroke-dasharray="${active?'2 3':'2 4'}"/><text x="${px}" y="${labelY}" text-anchor="middle" fill="${color}" stroke="#fffdfa" stroke-width="2" paint-order="stroke" font-size="${active?8:7}" font-weight="${active?900:800}">${point.port}</text></g>`;});
    ...
    const nktTimeMarkup=nktX>=L&&nktX<=R?`<g class="cepstrum-object-time" data-kind="nkt-time" aria-hidden="true"><line x1="${nktX}" y1="${B}" x2="${nktX}" y2="${B+4}"/><text x="${nktX}" y="${B+14}" text-anchor="middle">${nktTimeText}</text></g>`:'';
    return markup+'</g>'+nktTimeMarkup;
  }
  function drawPlot(svg,raw,{robust=false,color='#1e6c5b',xDomain=null,activeRange=null,wellMarkers=false}={}){
    if(!raw.length)return;const scaleState=cepstrumScaleStates.get(svg),options={robust,color,xDomain,activeRange,wellMarkers};if(scaleState){scaleState.raw=raw;scaleState.options=options;}
    ...
    const compact=svg.classList.contains('signal-plot'),w=Math.max(320,svg.clientWidth||svg.parentElement.clientWidth),h=compact?104:280,L=CEPSTRUM_PLOT_LEFT,R=w-CEPSTRUM_PLOT_RIGHT,T=compact?8:17,B=h-(compact?8:17),x=value=>L+(value-xmin)/(xmax-xmin||1)*(R-L),y=value=>B-(Math.max(ymin,Math.min(ymax,value))-ymin)/(ymax-ymin)*(B-T),clipId=`${svg.id}PlotClip`,activeClipId=`${svg.id}ActiveClip`,path=series=>series.map((point,i)=>`${i?'L':'M'}${x(point.x).toFixed(2)},${y(point.y).toFixed(2)}`).join(' '),activeStart=num(activeRange?.[0]),activeEnd=num(activeRange?.[1]),hasActiveRange=isNum(activeStart)&&isNum(activeEnd)&&activeEnd>activeStart,activeLeft=Math.max(L,Math.min(R,x(activeStart))),activeRight=Math.max(L,Math.min(R,x(activeEnd)));
    let markup=`<defs><clipPath id="${clipId}"><rect x="${L}" y="${T}" width="${R-L}" height="${B-T}"/></clipPath>${hasActiveRange&&activeRight>activeLeft?`<clipPath id="${activeClipId}"><rect x="${activeLeft}" y="${T}" width="${activeRight-activeLeft}" height="${B-T}"/></clipPath>`:''}</defs>`;
    if(scaleState)markup+=`<g class="cepstrum-time-boundaries" aria-hidden="true"><line x1="${L}" y1="${T}" x2="${L}" y2="${B+4}"/><line x1="${R}" y1="${T}" x2="${R}" y2="${B+4}"/><text x="${L}" y="${h-3}" text-anchor="start">${fmtTime(xmin,xmax-xmin)}</text><text x="${R}" y="${h-3}" text-anchor="end">${fmtTime(xmax,xmax-xmin)}</text></g>`;
    if(wellMarkers)markup+=wellObjectMarkerMarkup(svg,w,L,R,T,B,clipId,xmin,xmax);
    const linePath=path(data);if(hasActiveRange){markup+=`<path class="plot-context" d="${linePath}" clip-path="url(#${clipId})" fill="none" stroke="#7f8984" stroke-opacity=".58" stroke-width="1.3" vector-effect="non-scaling-stroke"/>`;if(activeRight>activeLeft)markup+=`<path class="plot-series" d="${linePath}" clip-path="url(#${activeClipId})" fill="none" stroke="${color}" stroke-width="1.3" vector-effect="non-scaling-stroke"/>`;}else markup+=`<path class="plot-series" d="${linePath}" clip-path="url(#${clipId})" fill="none" stroke="${color}" stroke-width="1.3" vector-effect="non-scaling-stroke"/>`;
    if(scaleState)markup+=`<g class="cepstrum-cursor-time" visibility="hidden" aria-hidden="true"><line x1="0" y1="${B-5}" x2="0" y2="${B+1}"/><g class="cepstrum-cursor-time-badge"><rect x="-32" y="${B+1}" width="64" height="15" rx="4"/><text x="0" y="${B+12}" text-anchor="middle"></text></g></g>`;
    svg.setAttribute('viewBox',`0 0 ${w} ${h}`);svg.innerHTML=markup;if(scaleState){scaleState.view={fullXmin,fullXmax,fullSpan,xmin,xmax,L,R,B,w,h};updateCepstrumCursorTimes();}
  }
  ...
  function fitCepstrumToWell(svg,state){
    const view=state?.view,velocity=+E.simVelocity.value;if(!M||!view||!(velocity>0))return;const nktX=projectWellDepth(svg,view.w,view.L,view.R,M.nkt),fraction=(nktX-view.L)/(view.R-view.L),travelTime=2*M.nkt/velocity;
    if(!(fraction>0)||!(travelTime>0)){error('НКТ должна находиться правее левой границы графика');return;}
    const scale=view.fullSpan*fraction/travelTime,offset=(view.xmin-view.fullXmin)/view.fullSpan;setCepstrumView(svg,state,{scale,offset});
  }
  ...
  function fullSignalDisplay(analysis){
    const {source,settings,bounds}=analysis,key=`${settings.low}:${settings.high}:${settings.interpolation}`;if(analysis.displayKey!==key){analysis.displayKey=key;analysis.displaySignal=processSignal(source,{start:bounds.xmin,end:bounds.xmax,low:settings.low,high:settings.high,interpolation:settings.interpolation})?.processed||[];}return analysis.displaySignal;
  }
  function renderSignalAnalysis(){
    analysisFrame=0;if(!currentSignalAnalysis)return;const {source,settings,bounds}=currentSignalAnalysis,result=analyzeSignal(source,settings),displaySignal=fullSignalDisplay(currentSignalAnalysis);
    ...
    setGraphState(E.computedSignal,E.computedSignalEmpty,false,'');E.computedSignalMeta.textContent=`${result.processed.length.toLocaleString('ru-RU')} отсчётов · ${windowText} · ${band} · ${interpolation}`;drawPlot(E.computedSignal,displaySignal,{color:'#586acb',xDomain:[bounds.xmin,bounds.xmax],activeRange:[settings.start,settings.end]});
  ...
  function simulateSensor(values,sourceRate,targetStep){const cutoff=Math.min(sourceRate*.45,.4/targetStep),filtered=zeroPhaseLowPass(values,cutoff,1/sourceRate);return{values:SIM_PROCESSING.resampleUniform(filtered,1/sourceRate,targetStep),cutoff};}
  function defaultPumpProfile(endDepth,velocity){const travel=2*endDepth/velocity,stop=12,minimumTime=stop+Math.max(2.5,travel*.45),reboundTime=minimumTime+Math.max(3,travel*.65);return{start:0,end:Math.max(48,reboundTime+24),stop,minimumTime,reboundTime,on:1,settled:.42,minimum:.23,rebound:.51,step:1/50};}
  function simulateSignal(test,ball,measuredSource=[]){
    const velocity=+E.simVelocity.value,noise=+E.simNoise.value/100,attenuation=.02+(+E.simAttenuation.value/100)*.22,width=+E.simPulse.value/1000,endDepth=ball?.depth??Math.max(...M.points.map(p=>p.depth)),ports=syncSimulationPorts(test,ball),activePort=+E.simPort.value,sourceRate=256,measuredProfile=estimatePumpProfile(measuredSource),profile=measuredProfile||defaultPumpProfile(endDepth,velocity),duration=Math.max(1,profile.end-profile.start),count=Math.ceil(duration*sourceRate)+1,envelope=new Float64Array(count),wave=new Float64Array(count),cepstrumWave=new Float64Array(count),reflectors=[];
    ...
    for(let i=0;i<count;i++){
      const time=profile.start+i/sourceRate;
      if(time<=profile.stop)envelope[i]=profile.on;
      else if(time<minimumTime){const phase=(time-profile.stop)/(minimumTime-profile.stop),smooth=phase*phase*(3-2*phase);envelope[i]=profile.on+(profile.minimum-profile.on)*smooth;}
      else{const elapsed=time-minimumTime;envelope[i]=profile.settled+slowAmplitude*Math.exp(-elapsed/slowTau)-fastAmplitude*Math.exp(-elapsed/fastTau);}
    }
    for(const point of ports)reflectors.push({depth:point.depth,amplitude:point.port===activePort?-.9:.14,type:point.port===activePort?'активная перфорация':'муфта'});if(M.nkt<=endDepth)reflectors.push({depth:M.nkt,amplitude:-.52,type:'конец НКТ'});if(ball)reflectors.push({depth:ball.depth,amplitude:1.15,type:'шар'});
    const pressureSpan=Math.max(1,Math.abs(profile.on-profile.minimum));for(const reflector of reflectors){const delay=2*reflector.depth/velocity,pathKm=2*reflector.depth/1000,amplitude=reflector.amplitude*pressureSpan*.025*Math.exp(-attenuation*pathKm);addRicker(wave,sourceRate,profile.stop-profile.start+delay,width,amplitude);addRicker(cepstrumWave,sourceRate,profile.stop-profile.start+delay,width,amplitude);}
    addRicker(cepstrumWave,sourceRate,profile.stop-profile.start+.08,width,-1.25*pressureSpan*.025);
    const random=seededRandom(`${M.file}:${test.id}:${activePort}`);let colored=0;for(let i=0;i<count;i++){const white=(random()+random()+random()+random()-2)*.5;colored=colored*.82+white*.18;const time=profile.start+i/sourceRate,pumpFactor=time<profile.stop?1:.45,value=noise*pressureSpan*.06*pumpFactor*(white*.65+colored*.35);wave[i]+=value;cepstrumWave[i]+=value;}
    const targetStep=Math.max(1/200,Math.min(.1,measuredProfile?.step||.02)),sensorEnvelope=simulateSensor(envelope,sourceRate,targetStep),sensorWave=simulateSensor(wave,sourceRate,targetStep),sensorCepstrumWave=simulateSensor(cepstrumWave,sourceRate,targetStep),outputCount=Math.min(sensorEnvelope.values.length,sensorWave.values.length,sensorCepstrumWave.values.length),source=[],residualSource=[];
    for(let i=0;i<outputCount;i++){const time=profile.start+i*targetStep;source.push([time,sensorEnvelope.values[i]+sensorWave.values[i]]);residualSource.push([time,sensorCepstrumWave.values[i]]);}
    const fullDepth=Math.ceil(Math.max(M.nkt,...M.points.map(point=>point.depth))/500)*500,reflectorDepths=reflectors.map(reflector=>reflector.depth),minDelay=2*Math.min(...reflectorDepths)/velocity,maxDelay=Math.max(2*endDepth/velocity,minDelay+targetStep*4),domainMax=Math.max(2*fullDepth/velocity,maxDelay);
    return{source,residualSource,sampleRate:1/targetStep,sensorCutoff:sensorEnvelope.cutoff,duration,reflectors:reflectors.length,activePort,endDepth,stopTime:profile.stop,endTime:source.at(-1)?.[0]??profile.end,fitted:Boolean(measuredProfile),minDelay,maxDelay,domainMax};
  }
  function drawSimulation(test,ball,measuredSource=[]){
    const simulation=simulateSignal(test,ball,measuredSource),analysis=analyzeSignal(simulation.residualSource,{start:simulation.stopTime,end:simulation.endTime,windowEdges:'right'},{floorRatio:1e-3,smoothBins:5}),rawCepstrum=analysis?.cepstrum||[],lifterTaper=Math.max(.08,(simulation.maxDelay-simulation.minDelay)*.08),liftedCepstrum=SIM_PROCESSING.lifterCepstrum(rawCepstrum,{min:simulation.minDelay,max:simulation.maxDelay,taper:lifterTaper,domainMax:simulation.domainMax}),cepstrum=SIM_PROCESSING.normalizeCepstrum(liftedCepstrum);
    setGraphState(E.simSignal,E.simSignalEmpty,false,'');setGraphState(E.simCepstrum,E.simCepstrumEmpty,!cepstrum.length,'Недостаточно данных для расчёта кепстра');
    E.simSignalMeta.textContent=`${simulation.source.length.toLocaleString('ru-RU')} отсчётов · остановка ${fmt(simulation.stopTime)} с · датчик ${fmt(simulation.sampleRate)} Гц / ≤ ${fmt(simulation.sensorCutoff)} Гц · ${simulation.fitted?'профиль по данным':'типовой профиль'}`;
    E.simCepstrumMeta.textContent=`нормированный отклик без огибающей · τ ${fmt(simulation.minDelay)}–${fmt(simulation.maxDelay)} с · −60 дБ · 5 бинов · порт ${simulation.activePort}`;
    drawPlot(E.simSignal,simulation.source,{color:'#c25a45'});if(cepstrum.length)drawPlot(E.simCepstrum,cepstrum,{robust:true,color:'#7256b5',wellMarkers:true});
  }
  ...
  document.addEventListener('click',event=>{const groupToggle=event.target.closest('.chart-group-toggle');if(groupToggle){const group=groupToggle.closest('.chart-group'),collapsed=group.classList.toggle('collapsed');groupToggle.setAttribute('aria-expanded',String(!collapsed));if(!collapsed&&M)requestAnimationFrame(()=>drawWell());return;}const wellScale=event.target.closest('.chart-well-scale');if(wellScale){const svg=wellScale.closest('.cepstrum-card').querySelector('.cepstrum-scale-target'),state=cepstrumScaleStates.get(svg);fitCepstrumToWell(svg,state);return;}const reset=event.target.closest('.chart-scale-reset');if(reset){const svg=reset.closest('.cepstrum-card').querySelector('.cepstrum-scale-target'),state=cepstrumScaleStates.get(svg);resetCepstrumView(svg,state);return;}const head=event.target.closest('.chart-head');if(!head||event.target.closest('.chart-view-controls')||event.target.closest('.signal-filter-controls,.simulation-controls'))return;const card=head.closest('.cepstrum-card'),collapsed=card.classList.toggle('collapsed');head.querySelector('.chart-toggle').setAttribute('aria-expanded',String(!collapsed));if(!collapsed&&M)requestAnimationFrame(()=>drawWell());});
  window.addEventListener('pointermove',event=>{if(event.pointerType==='touch')return;guideX=event.clientX;guideVisible=true;E.cursorGuide.classList.add('visible');if(!guideFrame)guideFrame=requestAnimationFrame(()=>{E.cursorGuide.style.transform=`translate3d(${guideX}px,0,0)`;updateCepstrumCursorTimes();guideFrame=0;});},{passive:true});document.documentElement.addEventListener('pointerleave',()=>{E.cursorGuide.classList.remove('visible');hideCepstrumCursorTimes();hideScaleTooltip();});window.addEventListener('blur',()=>{E.cursorGuide.classList.remove('visible');hideCepstrumCursorTimes();hideScaleTooltip();setZoomModifier(false);});
  ...
  E.simPort.addEventListener('change',()=>{redrawSimulation();if(!M)return;const test=M.tests[+E.select.value||0],ball=M.points.find(p=>p.port===test.ballPort);drawWellDiagram(ball);drawMiniWell(ball);for(const svg of [E.cep,E.computed]){const state=cepstrumScaleStates.get(svg);if(state?.raw.length&&state.options?.wellMarkers)drawPlot(svg,state.raw,state.options);}scheduleStickyState();});
  let stickyFrame=0;
  function updateStickyState(){
    ...
    E.cursorGuide.style.top=`${Math.max(0,Math.floor(wellStuck?headRect.bottom+WELL_GEOMETRY.miniHeight-2:diagramRect.bottom))}px`;
  }
