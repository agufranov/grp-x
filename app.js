(() => {
  ...
  const WELL_GEOMETRY={height:190,axis:150,miniHeight:80,miniAxis:40};
  let M;
  ...
  function drawMiniWell(ball){
    ...
    markup+=`<rect x="${L}" y="${sy(136)}" width="${R-L}" height="28" rx="14" fill="url(#miniPipe)"/><rect x="${ballX}" y="${sy(118)}" width="${Math.max(0,R-ballX)}" height="64" fill="#10191d" opacity=".68"/><rect x="${ballX}" y="${sy(118)}" width="${Math.max(0,R-ballX)}" height="64" fill="url(#miniHatch)"/><line x1="${L}" y1="${y}" x2="${nktX}" y2="${y}" stroke="#688df2" stroke-width="7"/><line x1="${nktX}" y1="${sy(125)}" x2="${nktX}" y2="${sy(175)}" stroke="#86a3f5" stroke-width="4"/><circle cx="${nktX}" cy="${y}" r="7" fill="#b9c9f7"/>`;
    for(const p of M.points){const px=x(p.depth),muted=ball&&p.depth>ball.depth,color=muted?'#68756f':'#c5e77b';markup+=`<line x1="${px}" y1="${sy(127)}" x2="${px}" y2="${sy(173)}" stroke="${color}" stroke-width="3"/><circle cx="${px}" cy="${y}" r="8" fill="${color}" stroke="#29343a" stroke-width="3"/>`;}
  ...
  function updateStickyState(){
    ...
    const diagramAxis=diagramRect.top+diagramRect.height*WELL_GEOMETRY.axis/WELL_GEOMETRY.height,stickyAxis=headRect.bottom+WELL_GEOMETRY.miniAxis;
