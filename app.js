(() => {
  ...
  function setGraphState(svg,empty,isEmpty,message){empty.textContent=message;empty.classList.toggle('hidden',!isEmpty);empty.parentElement.classList.toggle('empty-chart',isEmpty);if(isEmpty){svg.innerHTML='';const state=cepstrumScaleStates.get(svg);if(state){state.raw=[];state.options=null;state.view=null;}}}
  function wellObjectMarkerMarkup(svg,w,L,R,T,B,clipId,xmin,xmax){
    if(!M)return'';const test=M.tests[+E.select.value||0],ball=M.points.find(point=>point.port===test.ballPort),activePort=selectedSimulationPort(),deepest=Math.max(M.nkt,...M.points.map(point=>point.depth)),maxDepth=Math.ceil(deepest/500)*500,wellGeometry=wellHorizontalGeometry(E.svg.clientWidth,320,maxDepth),wellRect=E.svg.getBoundingClientRect(),plotRect=svg.getBoundingClientRect(),screenProjection=wellRect.width>0&&plotRect.width>0;
  ...
  function drawPlot(svg,raw,{robust=false,color='#1e6c5b',xDomain=null,activeRange=null,wellMarkers=false}={}){
    ...
    if(scaleState)markup+=`<g class="cepstrum-time-boundaries" aria-hidden="true"><line x1="${L}" y1="${B}" x2="${L}" y2="${B+4}"/><line x1="${R}" y1="${B}" x2="${R}" y2="${B+4}"/><text x="${L}" y="${h-3}" text-anchor="start">${fmtTime(xmin,xmax-xmin)}</text><text x="${R}" y="${h-3}" text-anchor="end">${fmtTime(xmax,xmax-xmin)}</text></g>`;
    ...
    svg.setAttribute('viewBox',`0 0 ${w} ${h}`);svg.innerHTML=markup;if(scaleState){scaleState.view={xmin,xmax,L,R,B,w,h};updateCepstrumCursorTimes();}
  }
